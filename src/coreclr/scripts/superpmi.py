    ...
    if include_details:
        ...
        with DetailsSection(write_fh, "Details"):
            if any_diffs:
                ...
                # def write_icount_row(name, base_metrics, diff_metrics, diffs):
                #     base_diff_icount = list(zip(base_metrics["Base InstructionCount"], diff_metrics["Diff InstructionCount"]))
                #     (num_improvements, num_regressions, num_same, byte_improvements, byte_regressions) = calculate_size_improvements_regressions(base_diff_icount)
                #     write_fh.write("|{}|{:,d}|{}|{}|{}|{}|{}|\n".format(
                #         name,
                #         len(diffs),
                #         html_color("green", "{:,d}".format(num_improvements)),
                ...
                # for (mch_file, base_metrics, diff_metrics, diffs, _, _) in asm_diffs:
                #     write_icount_row(mch_file, base_metrics, diff_metrics, diffs)

                # if len(asm_diffs) > 1:
                #     write_icount_row("", [r for (_, base_metrics, _, _, _, _) in asm_diffs for r in base_metrics], 
                #                          [r for (_, _, diff_metrics, _, _, _) in asm_diffs for r in diff_metrics],
                #                          [r for (_, _, _, diffs, _, _) in asm_diffs for r in diffs])
